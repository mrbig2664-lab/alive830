...
- Mobile interaction heights are explicit: unfolded quick records use 190px action cards with 118px resident art, mood controls use 126px minimum height, folded actions use 132px / 102px controls, and navigation cells use 120px minimum.
...
- Folded mode keeps the action order: room → today's focus → primary action → other records; it intentionally has no unfolded HUD, full quick-record row, or four-item navigation. Its fixed rhythm is 40% room, 22% focus, 23% actions, and 15% lower safe area, without content-height gaps. The room keeps a quieter bed/window/lamp/plant/support-object composition and scales residents from the ZhanZhan reference size.
